...
ENT.Type 			= "anim"
ENT.Base 			= "base_anim"
ENT.PrintName		= "Ships Cannon OnlyS"
...
if (CLIENT) then
	killicon.Add("psw_shipscannon", "killicons/sent_cball_killicon", Color(255,255,255,255))
...
if (SERVER) then
	...
	if owner:IsPlayer() && owner:Alive() then
		...
		if pang > 15 then
			ang.p = (bang - Angle(18,0,0)).p
		end
		if pang < (0 - 15) then
			ang.p = (bang + Angle(18,0,0)).p
		end
		if yang > 5 then
			ang.y = (bang - Angle(0,8,0)).y
		end
		if yang < (0 - 5) then
			ang.y = (bang + Angle(0,8,0)).y
		end
